...
#pragma pack(pop)
	...
	void GunCon2State::threadOutputs()
	{
		...
		Console.WriteLn("THREAD : Thread '%d' Started", port);
		
		//keep initialRatio
		AspectRatioType initialRatio = EmuConfig.CurrentAspectRatio;
		if(port == 1) Console.WriteLn("THREAD : initial Ratio '%d'", initialRatio);
		
		//keep initialStretch
		float initialStretch = GSConfig.StretchY;
		if(port == 1) Console.WriteLn(fmt::format("THREAD : initial Stretch '{}'", initialStretch));
		
		//to know if overlay has been switched by the thread or not
		bool isSwitchedToSecondOverlay = false;
		...
		while (VMManager::HasValidVM() && active_game != "" && !quitThread)
		{
			...
			//in fact we detect during cinematic/transition and gameplay the change
			//but when we exit game (start/C + trigger), if we bypass cinematic (when we are on NAMCO logo), we will not detect and keep same ratio/calibration
			//take care... it's not perfect especially on Times Crisis II, it's a hack ! ;-)
			//Time Crisis 2 EU
			...
			//to do only if 2 guns and for second one to avoid to run it 2 times
			if(port == 1){
				//Console.WriteLn(fmt::format("THREAD : splitscreen_activated '{}'", splitscreen_activated));
				//Console.WriteLn("THREAD : EmuConfig.CurrentAspectRatio '%d'", EmuConfig.CurrentAspectRatio);
				...
				if(splitscreen_activated && split_screen_hack){
					...
					if(EmuConfig.CurrentAspectRatio != AspectRatioType::R16_9){
					  ...
					  Console.WriteLn("THREAD : EmuConfig.CurrentAspectRatio '%d'", EmuConfig.CurrentAspectRatio);
					  if(!isSwitchedToSecondOverlay) needToSwitchOrDisplayOverlay = true;
					}
					if((!split_screen_full_stretch) && (GSConfig.StretchY == 100.0f)){
					  ...
					  Console.WriteLn(fmt::format("THREAD : GSConfig.StretchY '{}'", GSConfig.StretchY));
					  if(!isSwitchedToSecondOverlay) needToSwitchOrDisplayOverlay = true;
					}
					if(needToSwitchOrDisplayOverlay){
					  ...
					  isSwitchedToSecondOverlay = true;
					  Console.WriteLn(fmt::format("THREAD : isSwitchedToSecondOverlay = '{}'", isSwitchedToSecondOverlay));
					}
				}
				else if(!splitscreen_activated && split_screen_hack && isSwitchedToSecondOverlay){
					Console.WriteLn("THREAD : Restore initial ratio/stretch !");
					...
					int returnCode = std::system("(xdotool keydown F10; sleep 0.2; xdotool keyup F10)&");
					isSwitchedToSecondOverlay = false;
					Console.WriteLn(fmt::format("THREAD : isSwitchedToSecondOverlay = '{}'", isSwitchedToSecondOverlay));
				}
			}
			std::this_thread::sleep_for(std::chrono::milliseconds(10));
		}
		Console.WriteLn("THREAD : Thread '%d' Stopped", port);
	}
	...
	void GunCon2State::threadAutoConfigure()
	{
		...
		while (threadOutputLoaded == false)
		{
			...
			{
				Console.WriteLn("ThreadLOAD INIT - port: %d", port);
